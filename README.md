...
This code example demonstrates how to use the TCPWM counter on MCU. The counter is started by a PWM signal and work with a 200 kHz clock frequency. Enable the counter CC0 match interrupt, when counter up to CC0 match value generates the interrupt to toggle the user LED.


[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce240776-tcpwm-counter)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce240776-tcpwm-counter](https://github.com/Infineon/mtb-example-ce240776-tcpwm-counter)" application with the desired name "TcpwmCounter" configured for the *KIT_T2G_C-2D-6M_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G_C-2D-6M_LITE --app-id mtb-example-ce240776-tcpwm-counter --user-app-name TcpwmCounter --target-dir "C:/mtb_projects"
   ```
...
This design consists of the TCPWM counter, PWM, and user LED resources. Configure the PWM work with 25 MHz and output the PWM wave to the start counter. Configure this PWM to work with PWM Mode, Period set to 9999 and Run Mode set to continuous. Then, configure the Trigger Outputs with Trigger 0 Event set to PWM (line out) and Trigger 0 signal set to TCPWM[0] Group[0] 16-bit Counter 0 start. The counter work with 200 kHz and 60000 period value. Enable the CC0 and CC0 compare function and enable the CC0 match interrupt. It will generate an interrupt to toggle the user LED when counter up to CC0 match value 50000. Configure the user LED pin as strong drive input buffer off and output mode to indicate the counter CC0 match interrupt.
...
**Table 1. Application resources**
 ...
 GPIO (PDL)       | CYBSP_USER_LED1 | User LED to show the counter CC0 match interrupt
...
Resources  | Links
-----------|----------------------------------
Application notes  | [AN235305](https://www.infineon.com/assets/row/public/documents/10/42/infineon-an235305-getting-started-with-traveo-t2g-family-mcus-in-modustoolbox-applicationnotes-en.pdf) – Getting started with TRAVEO&trade; T2G family MCUs in ModusToolbox&trade;　<br> [AN220193](https://www.infineon.com/row/public/documents/10/42/infineon-an220118---gpio-usage-setup-in-traveo-t2g-family-applicationnotes-en.pdf) – GPIO usage setup in TRAVEO&trade; T2G family<br> [AN220224](https://www.infineon.com/row/public/documents/10/42/infineon-an220224---how-to-use-timer-counter-and-pwm-tcpwm-in-traveo-t2g-family-applicationnotes-en.pdf) – How to use Timer, Counter, and PWM (TCPWM) in TRAVEO&trade; T2G family <br> [AN241720](https://www.infineon.com/document-promo/infineon-an241720-getting-started-with-xmc5000-mcu-on-modustoolbox-software_1071f992-eb73-4dce-94ad-e84c41407bfc) – Getting started with XMC5000 MCU on ModusToolbox&trade; software
...
Document title: *CE240776* – *PDL: TCPWM counter* 
